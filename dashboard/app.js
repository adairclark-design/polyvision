...
// hasn't confirmed PRO yet (webhook timing lag). Polls every 2s for up to 30s.
async function pollForProStatus(attempts = 0) {
  if (attempts >= 15) {
    // 30 seconds elapsed — give up silently; user can refresh manually
    console.warn('[PolyVision] PRO polling timed out — webhook may be delayed. Try refreshing.');
    return;
  }
  // Show a floating activation banner on the first attempt
  if (attempts === 0) {
  ...
  try {
    ...
    if (resp.ok) {
      const data = await resp.json();
      if (data.is_pro === true) {
        // PRO confirmed — celebrate and clean up
        state.isProUser = true;
        state.discordLinked = !!data.discord_user_id;
        history.replaceState({}, '', location.pathname);
        document.getElementById('proActivatingBanner')?.remove();
...
window.startStripeCheckout = async function () {
  ...
  try {
    ...
    if (!resp.ok) throw new Error(await resp.text());
    const { url } = await resp.json();
    state.isProUser = undefined;   // reset so PRO status re-checks on return
