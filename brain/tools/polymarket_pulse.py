    ...
    for attempt in range(3):
        try:
            r = requests.get(
                ...
                params={"active": "true", "closed": "false", "limit": limit, "_sort": "volume24hr:desc"},
                timeout=10,
            )
            ...
            log.error(f"Gamma /markets fetch attempt {attempt+1} failed: {e}")
            time.sleep(5)
    return []
...
def fetch_recent_trades(condition_id: str, limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        log.debug(f"Data API /trades for {condition_id}: {e}")
    return []
...
def normalize_trade(raw: dict, market_title: str) -> dict:
    ...
    if isinstance(ts_raw, int) or (isinstance(ts_raw, str) and str(ts_raw).isdigit()):
        ts_iso = datetime.fromtimestamp(int(ts_raw), timezone.utc).isoformat()
    else:
        ts_iso = ts_raw or datetime.now(timezone.utc).isoformat()

    return {
        "id":             raw.get("transactionHash", "") or raw.get("id", ""),
        "market_id":      raw.get("conditionId", ""),
        "market_title":   raw.get("title", market_title),
        "outcome":        raw.get("outcome", str(raw.get("outcomeIndex", ""))),
        "price":          price,
        "size":           size,
        "usd_value":      round(usd_value, 2),
        "maker_address":  raw.get("proxyWallet", "") or raw.get("maker", ""),
...
def process_once(r: redis_lib.Redis, verbose: bool = False) -> list[dict]:
    ...
    for market in markets[:10]:  # Check top 10 active markets per cycle
        ...
        raw_trades = fetch_recent_trades(condition_id, limit=20)
