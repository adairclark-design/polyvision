...
def exchange_code_for_user_id(code: str, redirect_uri: str) -> str | None:
    """
    Complete Discord OAuth2 flow:
      1. Exchange authorization code for an access token
      2. Call /users/@me to get the user's Discord ID
    Returns the Discord user ID string, or None on failure.
    """
    client_id     = os.getenv("DISCORD_CLIENT_ID", "")
    client_secret = os.getenv("DISCORD_CLIENT_SECRET", "")
    if not client_id or not client_secret:
        log.error("DISCORD_CLIENT_ID or DISCORD_CLIENT_SECRET not set.")
        return None
    ...
    try:
        ...
        log.error(f"Discord token exchange failed: {e}")
        return None
    ...
    try:
        ...
        log.error(f"Discord /users/@me failed: {e}")
        return None
