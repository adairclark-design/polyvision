...
try:
    ...
    get_market_context = None  # type: ignore
...
# ── GPT-4o Summarizer ─────────────────────────────────────────────────────────
def build_prompt(payload: dict, extra_instruction: str = "", live_context: str = "") -> list[dict]:
    ...
    if live_context:
        user_msg += f"\n\n--- LIVE MARKET CONTEXT (use this to explain WHY) ---\n{live_context}"
    ...
    if live_context:
        system_msg += (
            "You MUST reference the live market context provided to explain WHY the whale "
            "made this move — what recent news or data triggered it. "
        )
    system_msg += (
        "Use Bloomberg Terminal tone — professional, urgent, analytical. "
...
def generate_summary(payload: dict, live_context: str = "") -> str:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            messages = build_prompt(payload, extra_instruction, live_context)
...
def summarize(payload: dict) -> dict:
    """Enriches a WhaleAlertPayload with an AI-generated summary + live context."""
    os.makedirs(".tmp", exist_ok=True)

    # Fetch live market context from Tavily (cached 5 min per market)
    market_title = payload.get("market_title", "")
    live_ctx_data = None
    live_context_str = ""
    if get_market_context and market_title:
        try:
            live_ctx_data = get_market_context(market_title)
            if live_ctx_data:
                live_context_str = live_ctx_data.get("context", "")
                log.info(f"Live context injected ({len(live_context_str)} chars) for '{market_title[:40]}'")
        except Exception as e:
            log.warning(f"market_context fetch failed (non-fatal): {e}")

    summary = generate_summary(payload, live_context_str)
    payload["ai_summary"] = summary

    # Attach context metadata for the dashboard to display
    if live_ctx_data:
        payload["live_context"]         = live_context_str
        payload["live_context_sources"] = live_ctx_data.get("sources", [])
        payload["live_context_fetched"] = live_ctx_data.get("fetched_at", "")
    else:
        payload.setdefault("live_context", None)
        payload.setdefault("live_context_sources", [])
