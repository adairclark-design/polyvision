...
def test_data_api():
    r = requests.get("https://data-api.polymarket.com/leaderboard?limit=1", timeout=10)
    r.raise_for_status()
...
def test_telegram():
    ...
    r.raise_for_status()
    assert r.json().get("ok"), "Telegram getMe returned ok=false"
...
if __name__ == "__main__":
    ...
    print("── Polymarket (Public) ──")
    check("Gamma API", test_gamma_api)
    check("Data API", test_data_api)
    ...
    print("\n── Community Channels ──")
    check("Discord Bot", test_discord)
    check("Telegram Bot", test_telegram)

    print("\n── Databases ──")
    check("Redis", test_redis)
    check("PostgreSQL", test_postgres)
